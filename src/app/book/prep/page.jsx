...
// Optional SEO: keep indexable or flip to noindex if you prefer.
...
export default function PrepPage({ searchParams }) {
  ...
  const refName = decodeURIComponent(searchParams?.n || "");
  const refEmail = decodeURIComponent(searchParams?.e || "");

  // success notice stays
  {sent && (
  ...
  // if sent, show referral card; else show the prep form
  if (sent && !shared) {
    return (
  ...
  // Wrap server action so we can redirect and show inline state via searchParams.
  async function action(formData) {
    ...
    if (res?.ok) {
      ...
      // You can expand this to encode specific field errors in the URL if desired.
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="mb-6 rounded-2xl border border-primary-100 bg-white p-4 shadow-sm">
        <div className="flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center">
          <p className="text-sm text-gray-700">
            Don’t have a free consultation scheduled yet?
          </p>
          <Link
            href="/book"
      ...
      <form action={action} className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                name="name"
                required
                className="w-full rounded-xl border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-400 outline-none ring-[--color-primary-100] focus:ring-2"
            ...
            <div>
              <label htmlFor="website" className="mb-1 block text-sm font-medium text-gray-700">
                Website URL <span className={`text-current/60`} >(or none)</span>
              </label>
              <input
                id="website"
                name="website"
        ...
        {/* Beliefs / Needs */}
        <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
          <h2 className="mb-4 text-lg font-semibold text-gray-900">What You Know You Need</h2>
          <textarea
            id="knows_need"
            name="knows_need"
            rows={4}
        ...
        <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
          <div className="flex items-start gap-3">
            <input
              ...
              type="checkbox"
              required
              className="mt-1 h-4 w-4 rounded border-gray-300 text-[var(--primary)]"
            />
          ...
          {/* Honeypot: keep visible to screen readers but hidden visually */}
