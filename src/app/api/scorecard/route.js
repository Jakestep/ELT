...
export async function POST(req) {
  const { name, email, url: rawUrl, goal = "", frustration = "" } = await req.json();
  if (!name || !email || !rawUrl) {
    return NextResponse.json({ ok: false, error: "Missing name, email, or website URL." }, { status: 400 });
  }
  ...
  await runInBackground({ name, email, url, goal, frustration }).catch(console.error); // TODO: remove await in production
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: "Couldn’t add you to our email list. Please try again." }, { status: 400 });
  }


  return new Response(JSON.stringify({ ok: true, message: "Generating your scorecard—check your email soon." }), {
    status: 202,
...
async function runInBackground({ name, email, url, goal, frustration }) {
  ...
  // Make PDF
  const pdfBuffer = await generatePdf({ name, email, url, goal, frustration, metrics, createdAt: new Date().toISOString() });
